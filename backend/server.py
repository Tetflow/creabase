...
async def get_next_invoice_number():
    """Get next sequential invoice number using atomic counter"""
    ...
    if not result:
        await db.counters.insert_one({"_id": "invoice_number", "sequence": 1})
        return 1
    
    return result["sequence"]

async def create_invoice_record(project_id: str, invoice_type: str, user_id: str, user_role: str, amount: float, project_data: dict):
    """Create an invoice record in database for tracking"""
    invoice_number = await get_next_invoice_number()
    formatted_invoice = f"INV-{invoice_number:06d}"  # Format: INV-000001, INV-000002, etc.
    
    invoice_record = {
        "invoice_id": formatted_invoice,
        "invoice_number": invoice_number,
        "invoice_formatted": formatted_invoice,
        "project_id": project_id,
        "project_title": project_data.get("title", ""),
        "invoice_type": invoice_type,  # "business_payment" or "creator_receipt"
        "user_id": user_id,
        "user_role": user_role,
        "amount": amount,
        "created_at": datetime.now(timezone.utc),
        "status": "issued",
        "fiscal_year": datetime.now(timezone.utc).strftime("%Y"),
        "fiscal_month": datetime.now(timezone.utc).strftime("%Y-%m")
    }
...
async def get_creators(
    ...
    creators = premium_creators + regular_creators
    
    for creator in creators:
        if isinstance(creator.get('created_at'), str):
            creator['created_at'] = datetime.fromisoformat(creator['created_at'])
        # Add premium flag
        if creator.get('premium_until') and isinstance(creator['premium_until'], datetime):
            creator['is_premium'] = creator['premium_until'] > now
        else:
            creator['is_premium'] = False
...
async def approve_project(project_id: str, current_user: User = Depends(get_current_user)):
    ...
    # Generate sequential invoices for both business and creator
    # Invoice 1: Business Payment Invoice
    business_invoice = await create_invoice_record(
        project_id=project_id,
        invoice_type="business_payment",
        user_id=project["business_id"],
        user_role="business",
        amount=escrow["business_pays"],
        project_data=project
    )
    
    # Invoice 2: Creator Receipt Invoice
    creator_invoice = await create_invoice_record(
        project_id=project_id,
    ...
    await db.notifications.insert_one({
        ...
        "message": f"Your work on '{project['title']}' has been approved. ₹{payout_amount} added to your wallet. Invoice: {creator_invoice}",
        "reference_id": project_id,
        "read": False,
        "created_at": datetime.now(timezone.utc)
    })
...
async def wallet_topup(topup_data: WalletTopUp, current_user: User = Depends(get_current_user)):
    """Add funds to wallet (Business users only)"""
    if current_user.role != "business":
        raise HTTPException(status_code=403, detail="Only business users can top up wallet")
    
    if topup_data.amount < 100:
    ...
    # Credit wallet
    new_balance = await update_wallet_balance(current_user.user_id, topup_data.amount, "credit")
    ...
    await add_wallet_transaction(
        ...
        topup_data.amount,
        "topup",
        f"Wallet top-up via {topup_data.payment_method}",
        payment_id,
...
async def request_withdrawal(withdrawal_data: WithdrawalRequest, current_user: User = Depends(get_current_user)):
    """Request withdrawal from wallet (Creator users only)"""
    if current_user.role != "creator":
        raise HTTPException(status_code=403, detail="Only creators can withdraw from wallet")
    ...
    creator = await db.creators.find_one({"submitted_by": current_user.user_id}, {"_id": 0})
    if not creator or not creator.get("bank_details"):
        raise HTTPException(status_code=400, detail="Please add bank details before withdrawal")
    ...
    await db.withdrawals.insert_one(withdrawal_request)
    
    # Debit wallet (hold amount)
    await update_wallet_balance(current_user.user_id, withdrawal_data.amount, "debit")
    ...
    await add_wallet_transaction(
        ...
        withdrawal_id,
        {"bank_account_id": withdrawal_data.bank_account_id, "status": "pending"}
    )
...
async def get_invoices(current_user: User = Depends(get_current_user)):
    """Get all invoices for current user with sequential invoice numbers"""
    
    # Get invoices from database based on user role
    if current_user.role == "creator":
        invoices = await db.invoices.find(
            {"user_id": current_user.user_id, "invoice_type": "creator_receipt"},
            {"_id": 0}
        ).sort("invoice_number", -1).to_list(100)
    elif current_user.role == "business":
        invoices = await db.invoices.find(
            {"user_id": current_user.user_id, "invoice_type": "business_payment"},
            {"_id": 0}
        ).sort("invoice_number", -1).to_list(100)
    else:
        raise HTTPException(status_code=403, detail="Only creators and businesses can view invoices")
    
    return invoices

@api_router.get("/invoices/{project_id}")
async def get_invoice(project_id: str, current_user: User = Depends(get_current_user)):
    """Generate detailed invoice for a project
    - Business: Payment Invoice (shows what they paid)
    - Creator: Receipt Invoice (shows what they received)
    """
    project = await db.projects.find_one({"project_id": project_id}, {"_id": 0})
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")
    ...
    if user_role == "creator":
        creator = await db.creators.find_one({"submitted_by": current_user.user_id}, {"_id": 0})
        if creator and creator["creator_id"] == project["creator_id"]:
            authorized = True
    ...
    creator = await db.creators.find_one({"creator_id": project["creator_id"]}, {"_id": 0})
    creator_user = await db.users.find_one({"user_id": creator["submitted_by"]}, {"_id": 0}) if creator else None
    
    # Generate unique invoice number
    invoice_prefix = "INV-B" if user_role == "business" else "INV-C"
    unique_suffix = project['project_id'][-8:].upper()
    timestamp = datetime.fromisoformat(str(project.get("completed_at", project.get("created_at")))).strftime('%Y%m%d') if project.get("completed_at") or project.get("created_at") else datetime.now(timezone.utc).strftime('%Y%m%d')
    invoice_number = f"{invoice_prefix}-{unique_suffix}-{timestamp}"
    
    # Build invoice based on user role
    if user_role == "business":
        ...
        invoice = {
            ...
            "invoice_date": project.get("completed_at", project.get("created_at")),
            "from": {
                "name": business_user.get("name", "Business User"),
                "email": business_user.get("email", ""),
                "role": "Business"
            },
            "to": {
                "name": "Creabase Platform",
                "email": "billing@creabase.com",
                "address": "Creabase Technologies Pvt Ltd"
            },
            "project": {
                "title": project["title"],
                "description": project["description"],
            ...
            "line_items": [
                ...
                },
                {
                    ...
                    "amount": project["fees"]["business_fee"]
                },
                {
                    "description": "GST (18% on fee)",
                    "amount": project["fees"]["business_gst"]
                }
            ],
            "subtotal": project["budget"],
            "platform_fee": project["fees"]["business_fee"],
            "gst": project["fees"]["business_gst"],
            "total": project["fees"]["business_pays"],
            "status": "paid" if project["status"] == "completed" else "pending",
            "payment_method": escrow.get("payment_method", "wallet") if escrow else "wallet"
        }
    else:
        # Creator Receipt Invoice
        invoice = {
            "invoice_number": invoice_number,
            ...
            "to": {
                "name": creator_user.get("name", "Creator") if creator_user else "Creator",
                "email": creator_user.get("email", "") if creator_user else "",
                "role": "Creator"
            },
            "project": {
                "title": project["title"],
            ...
            "gross_amount": project["budget"],
            "deductions": [],
            "total": project["fees"]["creator_receives"],
            "status": "paid" if project["status"] == "completed" else "pending",
            "payment_method": "wallet_credit"
        }
        
        # Add deductions if creator is not subscribed
        if not project["fees"].get("creator_has_subscription", False):
            invoice["deductions"] = [
                {
                    "description": "Platform Fee (10%)",
                    "amount": project["fees"].get("platform_earns_from_creator", 0) / 1.18  # Fee without GST
                },
                {
                    "description": "GST (18% on fee)",
                    "amount": project["fees"].get("platform_earns_from_creator", 0) - (project["fees"].get("platform_earns_from_creator", 0) / 1.18)
                }
            ]
    ...
    creator = await db.creators.find_one({"creator_id": project["creator_id"]}, {"_id": 0})
    creator_user = await db.users.find_one({"user_id": creator.get("submitted_by")}, {"_id": 0}) if creator else None
    escrow = await db.escrow_transactions.find_one({"project_id": project_id}, {"_id": 0})
    
    # Calculate amounts
    base_amount = project["budget"]
    platform_fee = escrow.get("platform_fee", base_amount * 0.1) if escrow else base_amount * 0.1
    gst_on_fee = platform_fee * 0.18  # 18% GST on platform fee
    total_platform_charge = platform_fee + gst_on_fee
    total_payable = base_amount + total_platform_charge
    creator_receives = base_amount - platform_fee
    
    invoice = {
        "invoice_number": f"INV-{project_id[-8:].upper()}-{datetime.now(timezone.utc).strftime('%Y%m')}",
        "invoice_date": project.get("completed_at", project.get("created_at")),
        "due_date": project.get("completed_at", project.get("created_at")),
        "status": "paid" if project["status"] == "completed" else "pending",
        
        # From (Creator/Service Provider)
        "from": {
            "name": creator.get("name", "Creator") if creator else "Creator",
            "email": creator_user.get("email", "") if creator_user else "",
            "phone": creator.get("phone", "") if creator else "",
            "address": f"{creator.get('city', '')}, {creator.get('state', '')}, India" if creator else "India"
        },
        
        # To (Business/Client)
        "to": {
            "name": business.get("name", business.get("email", "Business")) if business else "Business",
        ...
        "project": {
            ...
            "completed_date": project.get("completed_at"),
            "deliverable_url": project.get("deliverable_url", "")
        },
        
        # Amount Breakdown
        "line_items": [
            {
                "description": f"Content Creation Service - {project['title']}",
                "quantity": 1,
                "rate": base_amount,
                "amount": base_amount
            }
        ...
        "fees": [
            ...
            {
                "description": "GST on Platform Fee (18%)",
                "amount": round(gst_on_fee, 2)
            }
        ],
        
        "total_fees": round(total_platform_charge, 2),
        "total_amount": round(total_payable, 2),
        "creator_receives": round(creator_receives, 2),
        
        # Payment Details
        "payment": {
            "method": escrow.get("payment_method", "Wallet") if escrow else "Wallet",
...
async def request_revision(order_id: str, request: Request, current_user: User = Depends(get_current_user)):
    ...
    # Notify creator
    await NotificationService.create_notification(
        db, order.get("creator_id", ""),
        "Revision Requested",
        f"Client requested revision: {revision_notes[:50]}...",
        "order",
        f"/orders/{order_id}"
    )
...
async def create_dispute(dispute: DisputeCreate, current_user: User = Depends(get_current_user)):
    ...
    await NotificationService.create_notification(
        db, "admin",
        "New Dispute Raised",
        f"Dispute raised for project {dispute.project_id}",
        "dispute",
        f"/admin/disputes/{dispute_id}"
    )
    
    return {"message": "Dispute created. Admin will review shortly.", "dispute_id": dispute_id}
...
async def get_disputes(current_user: User = Depends(get_current_user)):
    if current_user.role == "admin":
        ...
        disputes = await db.disputes.find({"raised_by": current_user.user_id}, {"_id": 0}).to_list(100)
    return disputes

@api_router.patch("/disputes/{dispute_id}/resolve")
async def resolve_dispute(dispute_id: str, resolution: str, current_user: User = Depends(get_current_user)):
    if current_user.role != "admin":
        raise HTTPException(status_code=403, detail="Admin access required")
...
async def generate_invoice(project_id: str, current_user: User = Depends(get_current_user)):
    project = await db.projects.find_one({"project_id": project_id}, {"_id": 0})
    if not project:
        raise HTTPException(status_code=404, detail="Project not found")
    ...
    invoice = {
        ...
        "creator_id": project["creator_id"],
        "amount": project["budget"],
        "fees": project.get("fees", {}),
        "gst_number": "MOCK_GST_NUMBER",
...
async def get_wallet_transactions(
    ...
    try:
        ...
        wallet_id = wallet.get("wallet_id") or user_id
...
async def adjust_wallet_balance(
    ...
    """Manually adjust wallet balance (add or subtract funds)"""
    if current_user.role != "admin":
        raise HTTPException(status_code=403, detail="Admin access required")
    
    if adjustment.adjustment_type not in ["credit", "debit"]:
        raise HTTPException(status_code=400, detail="Invalid adjustment type. Use 'credit' or 'debit'")
    
    if adjustment.amount <= 0:
        raise HTTPException(status_code=400, detail="Amount must be positive")
    
    if not adjustment.reason or len(adjustment.reason.strip()) < 5:
    ...
    try:
        ...
        wallet_id = wallet.get("wallet_id", f"wallet_{user_id}")
        ...
        if adjustment.adjustment_type == "credit":
            ...
            if current_balance < adjustment.amount:
                raise HTTPException(
                    status_code=400, 
                    detail=f"Insufficient balance. Current: ₹{current_balance}, Requested: ₹{adjustment.amount}"
                )
            new_balance = current_balance - adjustment.amount
            transaction_type = "admin_debit"
            description = f"Admin debit: {adjustment.reason}"
        ...
        logger.info(
            f"Wallet adjusted: user={user_id}, type={adjustment.adjustment_type}, "
            f"amount={adjustment.amount}, admin={current_user.user_id}"
        )
        ...
        logger.error(f"Failed to adjust wallet: {str(e)}")
...
async def startup_event():
    ...
    try:
        ...
        if not existing_admin:
            ...
            logger.info(f"✅ Admin user created: admin@creabase.com")
        else:
...
async def get_all_invoices(
    ...
    """Get all invoices for admin with search and filter"""
    if current_user.role != "admin":
        raise HTTPException(status_code=403, detail="Admin access required")
    
    # Get all completed/paid projects
    query = {}
    if status != "all":
        query["status"] = status
    else:
    ...
    projects = await db.projects.find(query, {"_id": 0}).sort("completed_at", -1).to_list(1000)
    
    invoices = []
    for project in projects:
        # Get business and creator details
        business = await db.users.find_one({"user_id": project["business_id"]}, {"_id": 0, "name": 1, "email": 1})
        creator = await db.creators.find_one({"creator_id": project["creator_id"]}, {"_id": 0, "name": 1})
        escrow = await db.escrow.find_one({"project_id": project["project_id"]}, {"_id": 0})
        
        invoice_data = {
            "invoice_id": f"INV-{project['project_id'][-8:].upper()}",
            "project_id": project["project_id"],
            "project_title": project["title"],
            "business_name": business.get("name", business.get("email", "Unknown")) if business else "Unknown",
            "creator_name": creator.get("name", "Unknown") if creator else "Unknown",
            "date": project.get("completed_at", project.get("created_at")),
            "amount": project["budget"],
            "platform_fee": escrow.get("platform_fee", project["budget"] * PLATFORM_FEE_PERCENT) if escrow else project["budget"] * PLATFORM_FEE_PERCENT,
            "gst": (escrow.get("platform_fee", project["budget"] * PLATFORM_FEE_PERCENT) * GST_PERCENT) if escrow else (project["budget"] * PLATFORM_FEE_PERCENT * GST_PERCENT),
            "total_amount": escrow.get("business_paid", project["budget"]) if escrow else project["budget"],
        ...
        if search:
            ...
            if not any([
                search_lower in invoice_data["project_title"].lower(),
                search_lower in invoice_data["business_name"].lower(),
                search_lower in invoice_data["creator_name"].lower(),
                search_lower in invoice_data["invoice_id"].lower()
            ]):
                continue
...
@api_router.patch("/admin/invoices/{project_id}")
async def update_invoice(
    ...
    project = await db.projects.find_one({"project_id": project_id}, {"_id": 0})
    if not project:
        raise HTTPException(status_code=404, detail="Project/Invoice not found")
    
    # Update allowed fields
    update_fields = {}
    if "platform_fee_override" in update_data:
        # Update escrow with new platform fee
        new_platform_fee = float(update_data["platform_fee_override"])
        gst_on_fee = new_platform_fee * GST_PERCENT
        total_platform_charge = new_platform_fee + gst_on_fee
        
        escrow_update = {
            "platform_fee": new_platform_fee,
            "gst_on_fee": gst_on_fee,
            "business_paid": project["budget"] + total_platform_charge,
            "creator_receives": project["budget"] - new_platform_fee,
            "updated_at": datetime.now(timezone.utc).isoformat()
        }
    ...
    if "gst_override" in update_data:
        # Update GST rate (affects future invoices)
        pass  # GST is calculated, not stored
    
    if "notes" in update_data:
        update_fields["admin_notes"] = update_data["notes"]
...
# Admin Invoice Management
@api_router.get("/admin/all-invoices")
async def get_all_invoices(
    invoice_type: Optional[str] = None,
    current_user: User = Depends(get_current_user)
):
    """Admin: Get all invoices with project details"""
    if current_user.role != "admin":
        raise HTTPException(status_code=403, detail="Admin access required")
    ...
    invoices = await db.invoices.find(query, {"_id": 0}).sort("invoice_number", -1).to_list(1000)
    ...
    for invoice in invoices:
        ...
        # Get project details (already have project_id and project_title)
        project = await db.projects.find_one({"project_id": invoice["project_id"]}, {"_id": 0})
        if project:
            invoice["project_status"] = project["status"]
            invoice["project_budget"] = project["budget"]
...
async def get_all_payouts(
    ...
    # Enrich with user details and wallet balance
    for payout in payouts:
        ...
        if wallet:
            ...
            payout["sufficient_balance"] = wallet["balance"] >= payout["amount"]
